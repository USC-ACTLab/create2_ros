...
private:
        ...
        const create2_controller::TrajectoryState2D::ConstPtr& msg)
    {
        ...
        float v = v_r * cos(theta_e) + m_Kx * x_e; // clamp(v_r * cos(theta_e) + m_Kx * x_e, -0.25, 0.25);
        float w = w_r + v_r * (m_Ky * y_e + m_Ktheta * sin(theta_e)); // clamp(w_r + v_r * (m_Ky * y_e + m_Ktheta * sin(theta_e)), -2.5, 2.5);
